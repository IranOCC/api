...
import {
  PopulatedType,
  listAggregation
} from 'src/utils/helper/listAggregation.helper';
import { User } from 'src/user/schemas/user.schema';
import {
  EstateFavorite,
  EstateFavoriteDocument
...
export class EstatePublicService {
  ...
  async findOneBySlugOrID(id_or_slug: string, user: User) {
    const query = ObjectId.isValid(id_or_slug)
      ? { _id: new ObjectId(id_or_slug) }
      : { slug: id_or_slug };
    ...
    const estate = await this.estateModel
      ...
      .populate({
        ...
        populate: { path: 'icon', select: 'content' }
      })
      .populate({
        path: 'type',
        select: 'title slug icon',
      ...
      .populate({
        ...
        populate: [
          { path: 'avatar', select: 'alt title path' },
          { path: 'phone', select: 'value' }
        ]
      })
      ...
      .populate({
        ...
        populate: { path: 'logo', select: 'alt title path' }
      })
  ...
  findAll(pagination: PaginationDto, filter: any, sort: any) {
    const populate: PopulatedType[] = [
      ...
      //
      ['estatecategories', 'category', 'title slug', false],
    ...
    if (filter.documentType) {
      ...
      filter['documentType'] = {
        $in: filter.documentType.map((v: string) => new ObjectId(v))
      };
      // delete filter.documentType
    }
    if (filter.features) {
      if (typeof filter.features === 'string')
        filter.features = [filter.features];
      filter['features'] = {
        $in: filter.features.map((v: string) => new ObjectId(v))
      };
    ...
    if (filter.area) {
      ...
      filter['area'] = !!filter.area[1]
        ? { $gte: parseFloat(filter.area[0]), $lte: parseFloat(filter.area[1]) }
        : { $gte: parseFloat(filter.area[0]) };
    }
    if (filter.price) {
    ...
    if (filter.totalPrice) {
      if (typeof filter.totalPrice === 'string')
  ...
  myEstates(pagination: PaginationDto, filter: any, sort: any, user: User) {
    ...
    const project =
      'title excerpt area isConfirmed confirmedAt isRejected rejectedAt rejectedReason canBarter buildingArea roomsCount mastersCount constructionYear buildingArea floorsCount unitsCount floor withOldBuilding publishedAt createdAt code province city district';
    const virtualFields = {
      // all: { $count: "" }
    };
    const searchFields =
      'title excerpt content code province city district quarter alley address';
    if (!filter) filter = {};
