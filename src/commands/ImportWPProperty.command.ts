...
// CLI_PATH=./dist/cli.js npx nestjs-command import:wpProperty -s 0
...
export class ImportWPPropertyCommand {
    ...
    async create(
        ...
        const { data } = await this.http.get('https://iranocc.com/wp-json/rapp/v1/exportProperty', { params: { page: 1 } }).toPromise();
        ...
        for (let i = skip; i < _count + skip; i++) {
            ...
            const _data: CreateEstateDto = {
                ...
                district,
                quarter,
                location: !!p.location ? [p.location[0], p.location[1]].join("") : undefined,
